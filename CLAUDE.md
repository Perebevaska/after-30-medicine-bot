...
- **Статистика** (`menu:stats`): кнопки «📈 За 7 дней», «📊 Соблюдение за 30 дней» (adherence, `stats:adherence`) и «📆 План на 7 дней»; под отчётами история/план — кнопка «📄 Скачать PDF»
...
| F3 | `schedule_utils.py`, `database.py`, `handlers/stats.py` | **F3 реализована** — соблюдение режима (adherence) за 30 дней. Экран `stats:adherence` («📊 Соблюдение за 30 дней» в `/stats`): по каждому активному лекарству `taken / положено` с индикатором 🟢≥80/🟡≥50/🔴 + общий итог. Числитель — `get_taken_counts()` (status='taken' за период в TZ пользователя); знаменатель — `count_due_by_medication(rules, …, created_dates)` по расписанию (а не по `intake_log`), с клампом по `medications.created_at` (не штрафует за дни до создания). `_pct_color()` вынесён в `stats.py`. Тесты: `test_schedule_utils` (кламп) + `test_adherence_db`. ⚠️ деактивированные лекарства теряют `schedule_rules` → в расчёт не входят |
