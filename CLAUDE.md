...
| ~~30~~ | ~~`handlers/meds.py`~~ | ~~В `_show_edit_freq_type_step` для multi-dosage: (1) устаревшее сообщение "⚠️ Изменить расписание нельзя" — нужно убрать; (2) кнопка "Изменить способ приёма" логически не относится к блоку расписания~~ — ✅ исправлено: шаг выбора питания перенесён между Дозировкой Б и расписанием |
...
| ~~32~~ | ~~`.claude/CLAUDE.md`~~ | ~~Синхронизировать два CLAUDE.md~~ — ✅ исправлено: `.claude/` обновлён: structure, DB schema, schedule_rules, multi-dosage, key patterns |
| ~~33~~ | ~~`handlers/meds.py`, `utils.py`, `scheduler.py`, `timezone.py`~~ | ~~Аудит валидации входных данных~~ — ✅ исправлено: `escape_md()` в `utils.py`; лимиты NAME_MAX_LEN=50/DOSAGE_MAX_LEN=30 в 6 input-хендлерах; экранирование в ~15 точках отображения Markdown |
| ~~35~~ | ~~`handlers/timezone.py`~~ | ~~После установки часового пояса новому пользователю непонятно что делать~~ — ✅ исправлено: `show_main_menu` принимает `hint=`, после TZ setup показывает "Нажми 💊 Мои лекарства, чтобы добавить первое лекарство" |
| ~~36~~ | ~~`handlers/stats.py`~~ | ~~В `/stats` добавить план лекарств на неделю~~ — ✅ исправлено: кнопка "📆 План на неделю", показывает 7 дней вперёд с учётом frequency через `_rule_fires_today` |
